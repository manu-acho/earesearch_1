...
import { Mic, Database, Lock, Scale, GitBranch, Users, FileText, ArrowRight, Calendar, Target, Filter } from "lucide-react";
...
export default function ResearchPage() {
  ...
  const filteredThemes = themes.filter(theme => {
    ...
    if (selectedFilter === "featured") return theme.featured;
    return theme.status === selectedFilter;
  });
      ...
      <div className="container mx-auto px-4">
        ...
        <section className="mb-20">
          <div className="mb-12 animate-slide-in-left">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">Core Research Areas</h2>
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl font-bold mb-2 text-foreground group-hover:text-primary transition-colors">{area.title}</h3>
                    </div>
                  </div>
        ...
        <section className="mb-20">
          <div className="mb-12 animate-slide-in-left">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">Research Methodologies</h2>
            <p className="text-lg text-muted-foreground max-w-3xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {methodologies.map((methodology, index) => (
              <Card 
                ...
                className="p-6 border-border/50 backdrop-blur-sm bg-card/50 hover:border-primary/50 transition-all duration-300 animate-slide-up"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <h3 className="text-lg font-bold mb-4 text-foreground">{methodology.title}</h3>
        ...
        <section className="mb-20 bg-gradient-to-b from-primary/5 to-transparent rounded-2xl p-8 md:p-12">
          <div className="mb-8 animate-slide-up">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">Technical Focus Areas</h2>
            <p className="text-lg text-muted-foreground max-w-3xl">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[
              { title: "Low-Resource ASR", badge: "Speech Technologies", description: "Fine-tuning and quantization of speech models for African languages with limited training data.", color: "primary" },
              { title: "Practical ZK Proofs", badge: "Cryptography", description: "Implementing zero-knowledge circuits for supply chain privacy and regulatory compliance.", color: "accent" },
              { title: "Smart Contract Systems", badge: "Blockchain", description: "Designing scalable, secure contracts for tokenization, compliance, and finance.", color: "primary" },
              { title: "Offline-First Systems", badge: "Edge Computing", description: "Optimizing AI models for edge deployment without internet connectivity.", color: "accent" },
              { title: "EPCIS Integration", badge: "Data Standards", description: "Blockchain-EPCIS patterns for verifiable supply chain traceability.", color: "primary" },
              { title: "Agricultural Finance", badge: "DeFi", description: "On-chain mechanisms for working capital access and risk management.", color: "accent" }
            ].map((item, idx) => (
              <Card key={item.title} className="p-6 bg-card/80 backdrop-blur-sm border-border/50 hover:border-primary/50 transition-all duration-300 animate-fade-in hover:shadow-lg hover:shadow-primary/10" style={{ animationDelay: `${idx * 100}ms` }}>
                <Badge className="mb-3 bg-primary/10 text-primary hover:bg-primary/20 border-primary/20">{item.badge}</Badge>
        ...
        <section className="mb-20">
          <div className="mb-12 animate-slide-in-left">
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">Active Research Projects</h2>
            <p className="text-lg text-muted-foreground max-w-3xl">
          ...
          {loading ? (
            <div className="flex justify-center items-center py-20">
              <div className="animate-glow rounded-full h-12 w-12 border-4 border-primary border-t-transparent"></div>
            </div>
          ) : filteredThemes.length > 0 ? (
            <div className="grid gap-8 lg:grid-cols-2">
              {filteredThemes.map((theme, index) => (
                <Link key={theme.slug} href={`/research/${theme.slug}`}>
                  <Card className="p-8 hover:shadow-lg hover:shadow-primary/20 transition-all duration-300 hover:scale-[1.02] cursor-pointer h-full bg-gradient-to-br from-card/80 to-card/60 backdrop-blur-sm border-border/50 hover:border-primary/50 group animate-slide-up" style={{ animationDelay: `${index * 100}ms` }}>
                    <div className="flex items-start justify-between mb-4">
                      <Badge className="bg-primary/10 text-primary hover:bg-primary/20 border-primary/20">
                        {theme.researchType || "Research"}
                      </Badge>
                      <Badge variant="outline" className={
                        theme.status === "active" ? "border-primary text-primary" : 
                        theme.status === "completed" ? "border-accent text-accent" : 
        ...
        <section className="max-w-4xl animate-fade-in">
          <Card className="p-8 md:p-12 bg-gradient-to-br from-primary/10 to-accent/10 border-primary/20 backdrop-blur-sm">
            <Database className="w-12 h-12 text-primary mb-6" />
            <h2 className="text-2xl md:text-3xl font-bold mb-4 text-foreground">Open Science & Digital Public Goods</h2>
            ...
            <div className="flex flex-wrap gap-3">
              <Badge className="bg-primary/10 text-primary hover:bg-primary/20 border-primary/20">Open Source Models</Badge>
              <Badge className="bg-primary/10 text-primary hover:bg-primary/20 border-primary/20">Public Datasets</Badge>
              <Badge className="bg-primary/10 text-primary hover:bg-primary/20 border-primary/20">Reproducible Research</Badge>
              <Badge className="bg-primary/10 text-primary hover:bg-primary/20 border-primary/20">Knowledge Transfer</Badge>
            </div>
          </Card>
        </section>
      </div>
